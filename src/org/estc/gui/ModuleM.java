...
public class ModuleM extends JPanel {
	...
	public ModuleM() {
		...
		JLabel label = new JLabel("Gestion des Modules");
		...
		TableRowSorter<ModuleTableModel> sorter 
	    = new TableRowSorter<ModuleTableModel>(model);
		...
		btnModifier.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(row != -1){
					Module m = new Module((Integer)model.getValueAt(row, 0),(String) model.getValueAt(row, 1),(String) model.getValueAt(row, 2),(Integer) model.getValueAt(row, 3),(Integer) model.getValueAt(row, 4));
					FrmModifierModule frame = new FrmModifierModule();
					FrmModifierModule.remplirChamps(m);
					frame.setVisible(true);
					//model.setEditable(row);
					System.out.println(m.getCodeModule()+" modified");
				}else{
		...
		btnSupprimer.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int row = table.getSelectedRow();
				if(row != -1){
					Module m = new Module((Integer)model.getValueAt(row, 0),(String) model.getValueAt(row, 1),(String) model.getValueAt(row, 2), (Integer)model.getValueAt(row, 3));
					...
					System.out.println(m.getCodeModule()+" deleted");
				}else{
